...
def run_migrations():
    ...
    with app.app_context():
        ...
        # 2.5 moderation_status ustuni (har bir e'lon moderatsiyadan o'tishi kerak)
        try:
            result = db.session.execute(db.text("SHOW COLUMNS FROM profiles LIKE 'moderation_status'"))
            if not result.fetchone():
                print("📝 moderation_status ustuni qo'shilmoqda...")
                db.session.execute(db.text("ALTER TABLE profiles ADD COLUMN moderation_status VARCHAR(20) DEFAULT 'pending'"))
                db.session.execute(db.text("UPDATE profiles SET moderation_status = 'approved' WHERE moderation_status IS NULL OR moderation_status = ''"))
                db.session.commit()
                print("✅ moderation_status ustuni qo'shildi")
        except Exception as e:
            db.session.rollback()
            if 'Duplicate' not in str(e) and 'already exists' not in str(e).lower():
                print(f"⚠️ Migratsiya (moderation_status): {e}")
...
def run_bot():
    ...
    if telegram_app:
        try:
            ...
            telegram_app.run_polling(stop_signals=None, drop_pending_updates=True, allowed_updates=None)
        except Exception as e:
            import traceback
            error_msg = str(e)
            if 'Conflict' in error_msg or '409' in error_msg:
                print(f"⚠️ Bot polling conflict (boshqa instance ishlayapti). Bot xabarlarni yuborish ishlaydi.")
            else:
...
# Faqat birinchi worker'da botni ishga tushirish (polling conflict'ni oldini olish)
# Render.com da WORKER_ID mavjud bo'lsa, faqat worker-0 da botni ishga tushirish
worker_id = os.getenv('WORKER_ID', '0')
is_main_worker = worker_id == '0' or not os.getenv('GUNICORN_CMD_ARGS')

if telegram_app and is_main_worker and (env == 'production' or os.getenv('GUNICORN_CMD_ARGS')):
    # Faqat bir marta ishga tushirish uchun
    bot_thread = threading.Thread(target=run_bot, daemon=True)
    bot_thread.start()
    print(f"✅ Telegram bot production rejimida ishga tushdi! (Worker: {worker_id})")
