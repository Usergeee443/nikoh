...
def get_listings():
    ...
    # Faqat tarif muddati tugamagan foydalanuvchilarning e'lonlari (muddat tugasa feedda ko'rinmaydi)
    active_tariff_user_ids = db.session.query(UserTariff.user_id).filter(
        UserTariff.is_active == True,
        UserTariff.expires_at > now
    ).distinct()
    query = query.filter(Profile.user_id.in_(active_tariff_user_ids))
...
def get_listing_detail(user_id):
    ...
    # Shu foydalanuvchining boshqa aktiv va tasdiqlangan e'lonlari
