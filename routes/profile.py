...
def edit():
    ...
    if request.method == 'POST':
        try:
            ...
            profile.current_residence = (request.form.get('current_residence') or '').strip()[:200] or profile.current_residence
...
def _user_has_active_tariff(user):
    """Foydalanuvchida muddati tugamagan aktiv tarif bormi"""
    from models.tariff import UserTariff
    from datetime import datetime
    now = datetime.utcnow()
    return UserTariff.query.filter(
...
def list_my_listings():
    ...
    has_active_tariff = _user_has_active_tariff(user)
    return jsonify({
        'listings': [p.to_dict() for p in profiles],
        'profiles': [{'id': p.id, 'name': p.name or 'E\'lon', 'is_primary': p.is_primary, 'is_active': p.is_active, 'is_published': getattr(p, 'is_published', True), 'moderation_status': getattr(p, 'moderation_status', 'pending')} for p in profiles],
        'has_active_tariff': has_active_tariff
    })
...
def create_listing():
    ...
    try:
        profile = Profile(
            ...
            current_residence=(data.get('current_residence') or '').strip()[:200] or None,
        ...
        try:
            ...
            if is_pub:
                if not user.profile_completed:
                    return jsonify({'error': "To'liq profil to'ldiring", 'message': "E'lonni yoqish uchun to'liq profil kerak."}), 400
                if not user.has_active_tariff:
                    return jsonify({'error': 'Tarif kerak', 'message': "E'lonni yoqish uchun aktiv tarif kerak."}), 400
            profile.is_published = is_pub
            # Yangi e'lon moderatsiyadan o'tishi kerak; admin tasdiqlagach is_active=True
            profile.moderation_status = 'pending'
            if is_pub:
                profile.is_active = False  # Moderatsiya tasdiqlanguncha feedda ko'rinmaydi
            else:
        ...
        # Yangi e'lon adminlarga yuboriladi (moderatsiya kutilmoqda); tasdiqlangach feedda ko'rinadi
        if getattr(profile, 'moderation_status', None) == 'pending':
            try:
                import threading
                from telegram_bot import send_pending_listing_to_admins
                flask_app = current_app._get_current_object()
                threading.Thread(target=send_pending_listing_to_admins, args=(profile.id, flask_app), daemon=True).start()
            except Exception:
                pass
...
def update_listing(listing_id):
    ...
    profile.current_residence = (data.get('current_residence') or '').strip()[:200] or profile.current_residence
    ...
    if data.get('is_published') is not None:
        want_pub = _parse_bool(data.get('is_published'))
        if want_pub:
            if not user.profile_completed:
                return jsonify({'error': "To'liq profil to'ldiring", 'message': "E'lonni yoqish uchun to'liq profil kerak."}), 400
            if not user.has_active_tariff:
                return jsonify({'error': 'Tarif kerak', 'message': "E'lonni yoqish uchun aktiv tarif kerak."}), 400
        profile.is_published = want_pub
        # E'lon yoqilganda moderatsiyadan o'tishi kerak: pending, admin tasdiqlagach is_active=True
...
def toggle_listing_publish(listing_id):
    ...
    want_publish = publish if publish is not None else not profile.is_published
    # E'lonni yoqish (publish) uchun aktiv tarif talab qilinadi (asosiy ham, qo'shimcha ham)
    if want_publish:
        if not user.profile_completed:
            return jsonify({'error': "To'liq profil to'ldiring", 'message': "E'lonni yoqish uchun to'liq profil kerak."}), 400
        if not user.has_active_tariff:
    ...
    profile.is_published = want_publish
    # E'lon yoqilganda moderatsiyadan o'tishi kerak; admin tasdiqlagach is_active=True
    if profile.is_published:
        profile.moderation_status = 'pending'
        profile.is_active = False
        if not profile.activated_at:
    ...
    try:
        db.session.commit()
        invalidate_user_data_cache(user.id)
        return jsonify({'success': True, 'is_published': profile.is_published, 'is_active': profile.is_active, 'moderation_status': getattr(profile, 'moderation_status', None), 'message': 'E\'lon yoqildi — moderatsiyadan o\'tkazilgach ko\'rinadi.' if profile.is_published else 'E\'lon o\'chirildi'})
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500
